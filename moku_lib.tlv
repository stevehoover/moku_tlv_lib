...
\m4
   // ============================================================
   // Moku TL-Verilog Library
   //
   // Reusable macros for modeling Liquid Instruments Moku products
   // (Moku:Go multi-instrument mode). Instantiate these from a
   // template/design file (see moku_template.tlv).
   //
   // Provides:
   //   Instrument macros (each drives $out_a/$out_b from $in_a/$in_b):
   //     m4+const_instrument(_which, #_val), m4+half_instrument(_which), m4+shift_instrument(_which, #_amt),
   //     m4+cap_instrument(_which, #_amt), m4+lag_instrument(_which), m4+noise_instrument(_which),
   //     m4+schmitt_trigger_instrument(_which, #_high, #_low)
   //   Platform macro:
   //     m4+moku_go(/_top, #_from_bus1, #_from_bus2, #_to_bus1, #_to_bus2)
   // ============================================================

   // Colors used by the moku_go visualization.
...
// ======================================
// Instrument (or half-instrument) Macros
// ======================================

\TLV schmitt_trigger_instrument(_which, #_high, #_low)
   $out_['']_which[15:0] =
        $reset           ? 16'h8000 :
        ($in_a > #_high) ? 16'h7fff :
        ($in_a < #_low)  ? 16'h8000 :
